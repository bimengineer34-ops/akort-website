...
export interface Input {
  ...
  placeholder?: string;
  required?: boolean;
  /** Pre-filled/fixed value; also used for `type: 'hidden'` fields. */
  value?: string;
  /** File inputs only, e.g. ".pdf,.dwg,.jpg,.png". */
  accept?: string;
  /** Renders a native `<select>` instead of `<input>`. */
...
export interface Form {
  ...
  button?: string;
  description?: string;
  /** Where to POST the submission (e.g. a Web3Forms/Formspree endpoint). Without it the form does nothing. */
  action?: string;
  method?: 'post' | 'get';
}
